...
Améliorations :
    - Pas de token en dur (utilise KAGGLE_USERNAME/KAGGLE_KEY)
...
Usage :
    python extraction_project/script/extract_download.py
"""
...
def require_kaggle_env() -> tuple[str, str]:
    """Valide la presence de KAGGLE_USERNAME/KAGGLE_KEY."""
    username = os.getenv("KAGGLE_USERNAME", "").strip()
    key = os.getenv("KAGGLE_KEY", "").strip()

    invalid_username = username in ("", "votre_username", "your_username")
    invalid_key = key in ("", "votre_api_key", "your_api_key")

    if invalid_username or invalid_key:
        print("\nERREUR : Kaggle non configure dans l'environnement.")
        print("Renseignez KAGGLE_USERNAME et KAGGLE_KEY dans .env puis relancez make pull-data.")
        exit(1)

    return username, key

def authenticate() -> KaggleApi:
    """Authentification via variables d'environnement KAGGLE_USERNAME/KAGGLE_KEY."""
    username, key = require_kaggle_env()

    # Force la configuration Kaggle depuis l'environnement pour eviter
    # un fallback implicite vers ~/.kaggle/kaggle.json.
    config_dir = tempfile.mkdtemp(prefix="kaggle_env_")
