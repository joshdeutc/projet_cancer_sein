"""
Entraînement ResNet18 sur les mammographies RSNA — cible "normalité".
...
utilisée dans `train_resnet.py` (où les 4 vues d'un exam partagent le même label).

Usage:
    python -m fine_tuning.train_resnet_normalite
    python -m fine_tuning.train_resnet_normalite --epochs 100 --batch-size 4
"""
...
def _make_entries(
    ...
    """Aplatit la liste d'exams en (chemin, label_normalite).

    Le label est lu dans `label_lookup` indexé par (patient_id, image_id) — il vient
    du CSV, PAS de exam['cancer_label']. Une image absente du CSV est skippée.
    """
    entries = []
    skipped_no_file = 0
    skipped_no_label = 0
...
def train(
    ...
    # Split stratifié au niveau exam (même logique que le pipeline cancer)
    ...
    # Stats train (calculées une fois, mises en cache — partagées avec le run cancer
    # car elles ne dépendent que des images, pas du label).
    ...
    ax.set_title(f"Courbe ROC normalité — val set (meilleur epoch : {ckpt['epoch']})")
    ax.legend()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Entraîne ResNet18 sur le label normalité (RSNA).")
